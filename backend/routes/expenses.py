...
def add_new_expense(group_id: int, expense_data: ExpenseCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Adds a new expense to a group. Payer and split members must belong to group."""
    return create_expense(db, group_id, expense_data)
