...
def pay_debt(settlement_data: SettlementCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Records a payment between two users to settle debt."""
    return record_settlement(db, settlement_data)
